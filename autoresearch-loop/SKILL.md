...
| **B 并行探索(文献广搜主力)** | 广泛搜集/整合文献、复杂子问题 | **一条消息内并发多个 Explore/general-purpose sub agent**：每个负责一个子方向/一批关键词/一类来源（方法类/baseline类/反方证据/跨域类比），并行铺开广度；各自带回**可核验出处**，汇总去重落 findings.jsonl。这是"广搜靠 sub agent"的落点。 |
| C 实验运行 | 长算力作业 | 提交后立即**分钟级轮询**：自动诊断报错、修复、重交（正合 verl 多机训练，见 PROJECT.md §6/§7） |
| **D idea共推 + 验证(本项目升级)** | 整合后推新 idea / 每轮 QA | 推新 idea 这一步**必须结合 GPT**：与 Codex(GPT-5.5 xhigh) **共同推演(co-thinking)而非事后打分**——Claude 提候选 idea + Codex 独立从同一材料推自己的 idea/挑漏洞，多轮 `mcp__codex__codex`+`codex-reply` 辩论后综合。证据链审核同样用 `@codex-reviewer`。详见 projects 仓 CLAUDE.md §4.1。 |
