...
> **原则：先用合理偏小的 setting 快速迭代、锁定正确方向，确认有真实收益后再上量。** 别在方向都没验证清楚时就砸大模型/大数据/长 response——那是把有限算力烧在未知方向上。

- **小 setting 的标准是"小到迭代快、又大到结论可外推"**：
  - **别太小**：太小的模型/太短 step/太少数据会让信号被噪声淹没、或出现只在小规模存在的假象（小模型学不动 ≠ 方法无效）。要小到**还能看出方法间的相对差异**。
  - 典型小档（按本集群已有资源，见 PROJECT.md §3）：先用 **Qwen3-1.7B/4B 而非 30B**、**几百~2000 step 而非全程**、**数据子集**、**response length 取够覆盖任务的较短值**、batch/N 适当缩小。
- **可调小的维度（在合理范围内压）**：模型规模、训练 step、数据量、response/context length、rollout N、batch size、eval 集大小。**一次小实验通常同时压多个维度换速度**，但要清楚这会牺牲一部分可外推性。
- **"看到收益"的判据要先定**（呼应节点②）：相对 baseline 的提升要**超过 run 间方差**、多 seed 一致、非塌缩/非作弊；只在小档同时满足才算"方向对了"。**模糊的"看起来好一点"不构成上规模理由。**
- **上规模是一个 gate，不是默认动作**：决定上量前，走节点③（实验规划）+ 拉 `@codex-reviewer` 审"小档收益是否真实、是否会因规模变化而消失、值不值得这笔算力"，并经**人类 gate 批准大算力**（CLAUDE.md）。
- **参考别人怎么做**：上规模/设计消融前，**先查相关 paper 的消融(ablation)与 scaling 部分**——他们用什么规模验证、消融了哪些组件、哪些结论随规模变化。文献广搜走 §4.1（sub agent 并行）。借鉴成熟的小规模验证设定，少走弯路。
- **记录规模选择的理由**：每个实验在 `experiments/<id>/decision.md` 写清"为什么用这个 setting、想验证什么、看到什么才上规模"，避免事后忘记、或不自觉地用大算力补救一个没验证的方向。
